...
Each eligible Auto reward now computes Advantage from the old persistent reward EMA, then updates the EMA once with `beta = 0.05`. The ordering deliberately leaves a policy-update point between Advantage calculation and EMA assignment for the later adaptive-Health phase. Eligible updates increment the Auto-turn counter; terminal Auto rewards also update the persisted Enemy win/loss counters. Manual outcomes and cancelled measurements change none of these values. `W_H` remains fixed at `1.0` in this phase.
